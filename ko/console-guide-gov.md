...
NHN Cloud (공공기관용) 콘솔은 NHN Cloud 서비스를 이용하기 위한 관리 도구와 작업 창의 역할을 합니다.
여기에서는 NHN Cloud (공공기관용) 콘솔의 기본적인 설정과 사용 방법을 안내합니다.

NHN Cloud (공공기관용) 콘솔은 아래와 같은 기능을 제공합니다.
...
콘솔 접속 및 User Access Key 토큰 인증 방식의 Public API 호출 시 IP ACL이 적용됩니다. [인증 방식 지원 현황 보기](/Support-Status/ko/supported-authentication-methods-gov/)
...
* 개인정보 보호 설정
    * 조직/프로젝트 > 멤버 관리 > NHN Cloud 계정 / IAM 계정 > 멤버 목록 다운로드 기능
        * 설정 안 함 선택 시, 계정 목록을 다운로드할 수 있는 모든 멤버가 멤버 목록을 다운로드할 수 있습니다.
...
### NHN Cloud 계정(공공기관용) / IAM 계정 정책

| 구분 | [NHN Cloud](http://gov-nhncloud.com)계정 | IAM 계정 |
| --- | --- | --- |
...
| 콘솔 접근 | - NHN Cloud 콘솔([https://console.gov-nhncloud.com/](https://console.gov-nhncloud.com/)) 접근<br>- NHN Cloud 계정 ID/비밀번호로 로그인<br>- (선택) 2차 인증(이메일 또는 SMS) | - IAM 콘솔(https://조직도메인.console.gov-nhncloud.com/) 접근<br>- Dooray!, ERP 서비스는 해당 서비스 도메인으로 접근<br>- 조직의 OWNER(또는 ADMIN)가 설정한 ID/비밀번호로 로그인<br>- 조직에서 설정한 로그인 보안(2차 인증, 서비스별 설정) 인증 |
...
* IAM 계정은 조직 서비스별로 설정할 수 있는 역할이 다릅니다.
* NHN Cloud(공공기관용) 계정과 IAM 계정의 클라우드 서비스 역할은 아래와 같습니다.
...
| 역할 | 설명 |
| --- | --- |
| OWNER | 조직 생성, 조직 관리, 멤버 관리, 조직 역할 그룹 관리, 조직 서비스 관리, 결제 관리 <br> 프로젝트 관리 등 조직 전체에 대한 Create(생성),Read(읽기), Update(갱신), Delete(삭제) |
| ADMIN | 조직 관리, 멤버 관리, 조직 역할 그룹 관리, 조직 서비스 관리, 결제 관리 <br> 프로젝트 관리 등 조직 전체에 대한 Create(생성),Read(읽기), Update(갱신), Delete(삭제) |
...
| PROJECT NOTICE MANAGEMENT ADMIN | 프로젝트 알림 관리 Create(생성), Read(읽기), Update(갱신), Delete(삭제) <br> 프로젝트 멤버 Read(읽기) <br> 프로젝트 역할 그룹 Read(읽기) <br> 프로젝트 알림 수신 그룹 관리 Read(읽기) |
| PROJECT NOTICE MANAGEMENT VIEWER | 프로젝트 알림 관리 Read(읽기) <br> 프로젝트 역할 그룹 Read(읽기) <br> 프로젝트 알림 수신 그룹 관리 Read(읽기) |
...
| Certificate Manager | VIEWER | Certificate Manager 서비스 Read(읽기) 권한 |
...
알림 관리 기능은 NHN Cloud에서 발송하는 알림별로 고객이 직접 수신 받을 대상자와 알림 방법(Email, SMS, 웹훅)을 설정할 수 있는 기능입니다.
...
2. 알림 목록에서 수신 대상자를 변경할 알림을 찾아서 **수신 대상 수정 > 수정** 버튼을 클릭합니다.
    ...
    - 여러 알림의 수신 대상자를 한 번에 변경하려면 알림의 체크박스를 선택한 후, 알림 목록 상단의 **수신 대상 일괄 설정** 버튼을 클릭합니다.

3. 멤버, 알림 수신 그룹, 역할별로 **알림 수신 대상 및 알림 방법(Email, SMS)**을 선택합니다.
    - 웹훅은 웹훅을 지원하는 알림에 한해 같은 페이지의 **웹훅 설정** 영역에서 추가할 수 있습니다.
    - 알림별로 지원하는 알림 방법이 다릅니다.
    - 수신 대상으로 알림 수신 그룹을 추가할 경우 해당 그룹에 설정된 알림 방법(Email, SMS)과 각 알림에서 지원하는 알림 방법이 일치해야 해당 방법으로 알림을 수신할 수 있습니다.

4. **저장** 버튼을 클릭하여 설정 내용을 저장합니다.


<a id="notification-receiver-group-management"></a>
## 알림 수신 그룹 관리 { #notification-receiver-group-management }

알림 수신 그룹 관리는 NHN Cloud에서 발송하는 알림에 대한 수신 그룹을 설정하는 기능입니다.

Email, SMS 알림은 조직/프로젝트 역할 기반의 수신 대상 설정 및 멤버에 대한 수신 대상 설정을 통해 지원합니다.
웹훅 알림은 기본 웹훅과 사용자 정의 웹훅을 제공합니다.
해당하는 알림 수신 그룹은 **서비스**에서 알림 설정 시 사용할 수 있습니다.

<a id="create-notification-receiver-group"></a>
### 알림 수신 그룹 생성 방법 { #create-notification-receiver-group }
...
5. **수신 대상 직접 설정**
    ...
    - 멤버별로 Email, SMS 알림 설정을 지원합니다.

6. **웹훅 설정**
    - 알림 발송 시 사용자가 원하는 웹훅으로 설정할 수 있습니다.
    - 유형별로 기본 웹훅과 사용자 정의 웹훅을 제공합니다.  (각각 최대 5개까지 지원)

<a id="webhook-settings"></a>
### 웹훅 설정 { #webhook-settings }
1. 콘솔 조직, 프로젝트 상단 탭에서 **[알림 수신 그룹 관리]** 탭을 선택합니다.
2. 하단 **웹훅 설정** 메뉴의 **+웹훅 추가** 버튼을 선택합니다.
3. **웹훅 추가** 팝업에서는 유형별로 기본 웹훅과 사용자 정의 웹훅을 제공합니다.
    - 기본 웹훅
        - 기본 웹훅은 지원하는 서비스 알림 시에만 발송할 수 있습니다.
        - 웹훅 이름
...
- 사용자 정의 웹훅
    - 사용자 정의 웹훅은 지원하는 서비스 알림 시에만 발송할 수 있습니다.
    - 웹훅 이름
        - 설정할 사용자 정의 웹훅의 이름을 문자 제한 없이 최대 40자 이내로 입력할 수 있습니다.
        * 같은 알림 수신 그룹 내 웹훅 이름은 중복될 수 없습니다.
    * 서비스
        * 웹훅을 발송할 서비스를 선택할 수 있습니다.
        * 사용자 정의 웹훅을 제공하는 서비스만 선택할 수 있습니다.
    * 이벤트
        * 선택한 서비스가 제공하는 이벤트 중 웹훅으로 수신할 이벤트를 선택할 수 있습니다.
    * 수신 대상
  ...
  - 요청 데이터
      - 요청 데이터에는 선택한 서비스와 이벤트에서 제공하는 파라미터를 입력할 수 있습니다.
          - Ctrl 키 + 스페이스 바를 눌러 해당 서비스와 이벤트가 지원하는 파라미터를 확인할 수 있습니다.
  ...
  - 이름, 서비스, 이벤트, 수신 대상을 입력한 뒤 **확인** 버튼을 클릭해 웹훅을 생성합니다.
...
### 추가 질문 접수 { #submit-additional-questions }
...
### 문의 답변 내용 확인 { #check-response-to-inquiry }
