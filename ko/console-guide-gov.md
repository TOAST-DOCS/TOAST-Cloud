...
알림 수신 그룹 관리 기능은 NHN Cloud에서 발송되는 알림에 대한 수신그룹을 설정할 수 있는 기능입니다.
역할에 의한 수신그룹 설정 외에 조직/프로젝트 역할 기반의 수신 대상 설정 및 NHN Cloud 멤버(NHN Cloud 계정, IAM 계정)에 대한 수신 대상 직접 설정을 지원합니다.
웹훅 설정을 통해 기본 웹 훅과 커스텀 웹 훅을 제공합니다.


1. **알림 수신 그룹**을 생성하려면 조직 또는 프로젝트 메뉴에 **알림 수신 그룹 관리** 버튼을 클릭합니다.
2. + 알림 수신 그룹 추가 버튼을 클릭합니다.
3. **기본 정보** 란에 알림 수신 그룹명, 설명을 입력합니다.
    - **알림 수신 그룹명**은 한글, 영문자, 숫자, 특수 문자가 40자 까지 입력가능합니다.
    - **설명**은 알림 수신 그룹명의 구분을 쉽게 하기 위해 입력 제한 없이 최대 100자 까지 입력 할 수 있습니다.
...
5. **수신 대상 직접 설정**
    - 알림 수신 그룹 추가에 수신 대상을 직접 설정 할 수 있습니다.
    - NHN Cloud 계정 혹은 IAM 계정별 EMAIL, SMS 알림 설정을 지원합니다.

6. **웹훅 설정**
    - 알림 발송시 사용자가 원하는 웹훅으로 설정이 가능합니다.
    - 유형별로 기본 웹훅과 커스텀 웹훅을 제공합니다.  (5개 까지 지원)

### 웹훅 설정
1. 콘솔 조직, 프로젝트 상단탭에 **[알림 수신 그룹 관리]** 탭을 선택합니다.
2. 하단 **웹훅 설정** 메뉴의 [+웹훅 추가] 버튼을 선택 합니다.
3. **웹훅 추가** 팝업에서는 유형별로 기본 웹훅과 커스텀 웹훅을 제공합니다.
    - 기본 웹훅
        - 웹훅 이름
            - 설정할 기본 웹훅의 이름을 제한 없이 최대 40자 이내로 입력할 수 있습니다.
            - 같은 알람 수신 그룹 내 웹훅 이름은 중복될 수 없습니다.
        - 수신 대상
            - 웹훅을 수신할 URL을 입력할 수 있습니다.
        - 비밀 키
            - 사용에 따라 SecretKey 를 설정하여 기본 웹훅에 전송할 수 있습니다.
    - 커스텀 웹훅
        - 웹훅 이름
            - 설정할 기본 웹훅의 이름을 제한 없이 최대 40자 이내로 입력할 수 있습니다.
            * 같은 알람 수신 그룹 내 웹훅 이름은 중복될 수 없습니다.
        * 발송 대상
            * 웹훅을 발송할 대상(커스텀 대시보드 혹은 서비스)를 선택할 수 있습니다.
            * 커스텀 웹훅을 제공하는 메뉴 혹은 서비스만 선택할 수 있습니다.
        * 수신 대상
        ...
        - 요청 데이터
            - 요청 데이터에는 발송 대상에서 제공하는 파라미터를 입력할 수 있습니다.
                - Ctrl + Space 를 눌러 해당 발송대상이 지원하는 파라미터를 확인 할 수 있습니다.
                - 우측 하단 미리보기를 통해 제공되는 Value 를 확인 할 수 있습니다.
                    - `alertId: {alertId}`
                      `orgName : {orgName}`
        - 헤더
        ...
        - 이름, 발송 대상, 수신 대상 입력후 확인 버튼을 눌러 웹훅을 생성합니다.

4. 추가한 웹훅은 웹훅 설정 목록에서 확인할 수 있습니다.
    * [보기] 버튼을 눌러 웹훅 상세 정보를 확인할 수 있습니다.
    * 연필 아이콘을 클릭해 웹훅 정보를 수정할 수 있습니다.
    * 휴지통 아이콘을 클릭해 웹훅을 삭제할 수 있습니다.

5. 생성된 웹훅은 웹훅 발송을 제공하는 서비스에 **알림 수신 그룹**을 추가해 사용 할 수 있습니다.