...
| 콘솔 접근 | \- NHN Cloud\(공공 기관용\) 콘솔\([https://gov-console.toast.com/](https://gov-console.toast.com/) 접근<br>\- NHN Cloud\> 회원 ID/비밀번호로 로그인<br>\- \(선택\) 2차\(이메일 또는 SMS\) 인증 | \- IAM 콘솔\([https://조직도메인.console.toast.com/](https://%EC%A1%B0%EC%A7%81%EB%8F%84%EB%A9%94%EC%9D%B8.console.toast.com/) 접근<br>\- \(Dooray\!\, ERP 서비스는 해당 서비스 도메인으로 접근\)<br>\- 조직의 OWNER\(또는 ADMIN\)가 설정한 ID/PW로 로그인 \- 조직에서 설정한 로그인 보안\(2차 인증\, 서비스별 설정\) 인증 |
