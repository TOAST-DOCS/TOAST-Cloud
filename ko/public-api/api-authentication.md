...
- 토큰 시간은 기본 1시간 동안 유효하며, 이 시간은 제공되는 프레임워크 API를 통해 변경할 수 있습니다. 변경 시, 다음에 발급되는 토큰부터 해당 만료 시간이 반영됩니다.
...
#### 토큰 발급
##### 사전 작업
아래 방법을 따라 User Access Key ID와 secret key를 우선 발급해야 합니다.
  1. 로그인 후 오른 쪽 상단의 메일 주소 > `API 보안 설정`
  2. `User Access Key ID 생성`
  3. User Access Key ID와 Secret Access Key 발급
     - 발급 후 **Secret Access Key는 복사하여 따로 보관**해야 합니다.
     - User Access Key ID는 `API 보안 설정` 페이지 목록에서 확인 가능합니다.
##### 2. 토큰 발급 요청
...
* 요청
    ...
    | Header        |  **Authorization** | **String**| **Yes** | Basic Base64(UserAccessKeyID:SecretAccessKey) | `UserAccessKeyID:SecretAccessKey` 를 Base64 인코딩한 결과를 `Basic ` 뒤에 붙여서 사용합니다. || 
...
* 케이스 별 요청 예시
    * curl
      * Header 에 인증 정보를 넣는 경우
        ...
        ```
        * 참고: `dXNlckFjY2Vzc0tleTp1c2VyU2VjcmV0S2V5`는 `UserAccessKeyID:SecretAccessKey`를 base64 encoding 한 결과입니다.
      * `-u` 옵션을 사용하는 경우
        ```sh
        curl --request POST 'https://oauth.api.nhncloudservice.com/oauth2/token/create' \
          -H 'Content-Type: application/x-www-form-urlencoded' \
...
* 토큰 생성/만료 API 호출 시 반환될 수 있는 에러코드
  * [The OAuth 2.0 Authorization Framework](https://datatracker.ietf.org/doc/html/rfc6749#section-5.2)와 동일한 에러코드를 반환합니다.
* 토큰 사용 시 반환될 수 있는 에러 코드
  * 에러 코드는 [프레임워크 API](framework-api.md)에 있습니다.