...
NHN Cloud Public API는 API마다 서로 다른 인증 방식을 지원합니다. [인증 방식 지원 현황](https://docs.{% if "gov" in build_flags %}gov-{% endif %}nhncloud.com/ko/nhncloud/ko/public-api/supported-authentication-methods)에서 각 API가 제공하는 인증 방식을 확인한 뒤, 해당 인증 방식을 사용해 API 요청을 인증하세요.
