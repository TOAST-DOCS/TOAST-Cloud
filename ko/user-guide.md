...
개인 회원은 간단한 이메일 인증과 SMS 인증을 통해 가입합니다.
사업자 회원은 사업자 정보를 기반으로 가입이 가능합니다.
TOAST 회원 관리 정보와 가입 절차는 아래와 같습니다.

### 회원 관리 정보
TOAST 회원은 서비스 제공 주체에 따라 한국 회원과 일본 회원으로 분리됩니다. 
한국 회원은 한국 법령에 의한 서비스 약관과 개인정보처리방침에 동의한 회원으로 NHN 에서 제공하는 서비스를 이용하는 회원입니다. 회원 가입시 국가 정보를 "한국", "Other" 를 선택한 경우 한국 회원으로 가입하게 됩니다. 
일본 회원은 일본 법령에 의한 서비스 약관과 개인정보차리방침에 동의한 회원으로 NHN Japan 에서 제공하는 서비스를 이용하는 회원입니다. 회원 가입시 국가 정보를 "일본" 으로 선택한 경우 일본 회원으로 가입하게 됩니다. 

한국 회원과 일본 회원에 대한 상세한 관리 내역은 아래와 같습니다. 

#### 한국 회원
| 관리 정보 | 설명 | 필수 입력 |
| --- | --- | --- |
| ID | - 이메일 수신이 가능한 이메일 주소입니다 (이메일 인증 필수). | O |
| 비밀번호| - 영문, 숫자, 특수문자 중 2개 이상을 조합하고 전체 8자 이상으로 구성합니다. <br/> - 이전과 동일한 비밀번호는 사용할 수 없습니다. <br/> - 대소문자를 구분합니다.  | O |
| 국가 | 회원의 국가 정보입니다. | O |
| 회원 가입 유형 | 1. 개인 <br/>         - 간편한 이메일 인증, 휴대폰SMS인증으로 회원 가입을 합니다. <br/> - 개인 회원은 결제 수단으로 신용카드를 사용할 수 있습니다. <br/> - 외국인의 경우, 개인 회원으로 가입이 가능합니다. <br/> 2. 사업자 <br/> - 회원 가입 시, 추가적으로 사업자등록 정보에 대해 인증을 진행합니다. <br/> - 사업자 회원은 결제 수단으로 신용카드와 계좌이체를 이용할 수 있으며, 계좌이체 이용 시, 가입 시 등록된 사업자 정보를 기반으로 세금계산서 발행이 가능합니다.  | O |
| 휴대폰 번호 | - SMS 확인이 가능한 휴대폰 번호입니다(휴대폰 SMS 인증 필수). | O |
| 이름 | - TOAST 서비스에서 사용할 사용자 이름입니다.  | O |
| 주소 | - 우편번호, 기본 주소, 상세 주소로 구성합니다.  | X |
| 유선전화 | - 빠른 서비스 지원을 위해 관리합니다.  | X |
| 사업자 정보 | -  사업자 회원의 경우 필수 관리 항목입니다. <br/> - 회사명, 사업자등록번호, 대표자명, 업종/업태, 주소, 유선전화 정보를 관리합니다. <br/> - 사업자등록증에 기재된 정보와 일치해야 합니다.  | O |
...
국가 정보에 따라 회원 정보 및 이용할 수 있는 결제 수단이 달라집니다.
...
- 홍보성 정보 수신 동의 (선택): TOAST 에서 시행하는 이벤트, 프로모션, 서비스 홍보 등의 정보를 수신하는 데 동의하는 것으로, 선택 동의 사항입니다. 홍보성 정보 수신 동의는 회원 가입 후 회원 정보에서 수정할 수 있습니다.
...
회원 가입이 완료되면 자동으로 로그인됩니다. TOAST 서비스 사용이 가능한 상태로 TOAST 홈페이지와 콘솔을 이용할 수 있습니다.

## 결제 수단 등록
TOAST 서비스를 이용하기 위해서는 결제 수단 등록이 필요합니다.
단, 기존에 만들어진 조직, 프로젝트의 멤버인 경우에는 결제 수단 등록 없이 TOAST 서비스를 이용할 수 있습니다.
결제 수단은 [마이 페이지 > 결제수단]에서 등록할 수 있습니다.
...
#### 일본 회원
- 신용카드: 카드사에서 제공하는 인증 정책에 따라 인증(카드 유효성 검증)을 하고, 신용카드를 결제 수단으로 등록합니다. 
...
| 일본 | 개인 | 신용카드 | 카드 소유 인증 |카드 번호, 유효기간, 이름, 보안코드 | 카드 유효성 검증을 위해 2엔(円) 결제 후 취소 처리 |

#### 한국 회원
##### PAYCO 신용카드
###### 결제 수단 선택
- **마이 페이지 > 결제수단** 화면에서 **결제 수단 변경** 버튼을 클릭합니다.
...
크레딧은 현금 1원과 동일합니다.
크레딧은 **마이페이지 > 크레딧**에서 확인할 수 있습니다.
크레딧은 아래와 같은 정책으로 운영됩니다.
- 환불되지 않습니다.
- 유효 기간을 관리합니다. 유효 기간 이후 소멸하며 적립 유형별로 유동적으로 운영합니다.
...
- 이용 요금이 남이 있는 경우, 자동 결제일(매월 8일)에 결제한 후 회원 탈퇴가 처리됩니다.

### 회원 탈퇴 완료
회원 탈퇴가 완료되면, 회원 탈퇴 신청 시 등록된 이메일로 탈퇴 내역이 발송됩니다.
회원 탈퇴 시 모든 이용 정보가 삭제되며, 삭제된 데이터는 복구되지 않습니다.
