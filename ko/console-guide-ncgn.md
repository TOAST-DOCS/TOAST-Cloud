...
| 콘솔 접근 | - NHN Cloud 콘솔(https://console.gncloud.go.kr) 접근<br>- NHN Cloud 계정 ID/비밀번호로 로그인<br>- (선택) 2차 인증(이메일 또는 SMS) | - IAM 콘솔(https://조직 도메인.console.gncloud.com/) 접근<br>- Dooray!, ERP 서비스는 해당 서비스 도메인으로 접근<br>- 조직의 OWNER(또는 ADMIN)가 설정한 ID/비밀번호로 로그인<br>- 조직에서 설정한 로그인 보안(2차 인증, 서비스별 설정) 인증 |
...
* [기본 인프라 서비스(Infrastructure)](../ko/overview-ncgn.md)의 역할별 상세 권한은 [전체 권한 매트릭스 보기](../etc/infrastructure_roles_guide-ncgn.html){:target="_blank" rel="noopener"}을 참조하세요.
