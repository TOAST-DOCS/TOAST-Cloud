...
NGSC은 NGSC 서비스를 이용하기 위한 관리 툴과 작업 창의 역할을 합니다.
여기에서는 NGSC 콘솔의 기본적인 설정과 사용 방법을 안내합니다.

NGSC은 아래와 같은 기능을 제공합니다.
...
조직은 NGSC 서비스를 효율적으로 사용하고 관리하기 위해 만들어진 그룹입니다.
조직에서는 동일한 서비스 정책을, 사용자에게 공유하여 사용할 수 있습니다.

### 조직 생성

* NGSC 서비스를 이용하기 위해서는 조직을 생성해야 합니다.
...
5. **설정** 버튼을 클릭하여 생성된 조직 정보를 확인합니다. 조직의 추가 정보로 도메인 정보를 입력합니다. 도메인은 NGSC에서 유일한 값으로 설정해야 합니다.
...
NGSC 서비스를 안정하고 효율적으로 이용하기 위해 필요한 정책을 설정하여 관리할 수 있습니다. 로그인 및 개인정보 등 보안 컴플라이언스 준수를 위한 조직의 공통된 정책을 수립하여 조직 내 멤버가 정책을 준수할 수 있도록 관리합니다.
...
프로젝트는 조직 생성 후, NGSC 서비스를 이용하기 위해서 생성합니다.
...
| 구분 | [NGSC](http://ngsc-nhncloud.com)회원 | IAM 멤버 |
| --- | --- | --- |
| 정의 | - 조직 관리를 위한 멤버<br>- NGSC 이용 약관에 동의한 NGSC회원으로, 서비스 이용에 대한 책임과 의무를 가지는 멤버<br>- NGSC서비스 전체에서 유효한 멤버로 소속된 조직이 삭제되어도 NGSC 회원으로 존재 | - 서비스 이용을 위한 멤버<br>- NGSC 이용 약관에 동의하지 않은 멤버<br>- 조직 내에서만 유효한 멤버, 소속된 조직이 삭제되면 삭제되는 멤버 |
| 멤버 등록 방법 | - 조직의 OWNER나 ADMIN이 NGSC ID를 입력하여 등록 | - 조직의 OWNER나 ADMIN이 조직 내 유일한 ID를 입력하여 등록<br>- SSO 연동/API 연동 등을 통해 등록 |
| 멤버 역할 | - 조직 관리(조직 생성/수정/조직 멤버 관리/조직 서비스 관리/결제 관리)<br>- 프로젝트 생성<br>- 프로젝트 삭제 | - 조직 서비스 이용 |
| 콘솔 접근 | - NGSC 콘솔 (https://console.ngsc-nhncloud.com/) 접근<br>- NGSC > 회원 ID/비밀번호로 로그인<br>- (선택) 2차(이메일 또는 SMS) 인증 | - IAM 콘솔(https://조직도메인.console.ngsc-nhncloud.com/) 접근<br>- 조직의 OWNER(또는 ADMIN)가 설정한 ID/PW로 로그인 - 조직에서 설정한 로그인 보안(2차 인증, 서비스별 설정) 인증 |


### 조직 멤버

* NGSC 회원과 IAM 멤버의 클라우드 서비스 역할은 아래와 같습니다.
...
NGSC서비스 이용 요금을 확인하고, 결제할 수 있습니다.
**내 정보 보기 > 결제 관리** 메뉴에서 결제 수단을 등록한 NGSC 회원의 청구서와 결제 예정 금액, 사용량 정보를 확인할 수 있습니다.
