...
* 個人情報保護設定
    * 組織/プロジェクト > メンバー管理 > NHN Cloud アカウント / IAM アカウント > メンバーリストダウンロード機能
        * **設定なし**を選択した場合、アカウントリストをダウンロードできるすべてのメンバーが、メンバーリストをダウンロードできます。
        * **設定**を選択した場合、メンバーリストダウンロード機能が無効化され、例外的に許可されたIPまたはIP帯域からのみメンバーリストのダウンロードが可能です。
    * 組織 > CloudTrail > 個人情報
        * **設定なし**を選択した場合、ログリストを照会できるすべてのメンバーに、ログリスト内のすべての情報を提供します。
        * **設定**を選択した場合、ログリスト内の個人情報（メールアドレス、名前、ID）がマスキング処理されて提供されます。
...
| PROJECT NOTICE MANAGEMENT ADMIN | プロジェクト通知管理 Create（作成）、Read（読み取り）、Update（更新）、Delete（削除） <br> プロジェクトメンバー Read（読み取り） <br> プロジェクトロールグループ Read（読み取り） <br> プロジェクト通知受信グループ管理 Read（読み取り） |
| PROJECT NOTICE MANAGEMENT VIEWER | プロジェクト通知管理 Read（読み取り） <br> プロジェクトロールグループ Read（読み取り） <br> プロジェクト通知受信グループ管理 Read（読み取り） |
...
通知管理機能は、NHN Cloudから送信される通知ごとに、お客様が直接受信対象者と通知方法（Email、SMS、Webフック）を設定できる機能です。

1. **組織 > 通知管理**または**プロジェクト > 通知管理**をクリックします。
    - 組織、プロジェクトごとに受信する通知を管理できます。

2. 通知リストで受信対象者を変更する通知を見つけて、**[受信対象の修正] > [修正]** ボタンをクリックします。
    - 左側のリストで通知を選択するか、右上の検索エリアで通知名、受信対象などを検索して通知を見つけることができます。
    - 複数の通知の受信対象者を一度に変更するには、通知のチェックボックスを選択した後、通知リスト上部の **[受信対象の一括設定]** ボタンをクリックします。

3. メンバー、通知受信グループ、ロール別に**通知受信対象および通知方法（Email、SMS）**を選択します。
    - WebフックはWebフックをサポートする通知に限り、同じページの**Webhook設定**エリアから追加できます。
    - 通知ごとにサポートする通知方法が異なります。
    - 受信対象として通知受信グループを追加する場合、該当グループに設定された通知方法（Email、SMS）と各通知でサポートする通知方法が一致している必要があります。一致している場合にのみ、その方法で通知を受信できます。
...
Email および SMS 通知は、組織/プロジェクトのロールベースの通知受信対象設定と、メンバーに対する通知受信対象設定を通じてサポートされます。
Webフック通知は、デフォルトの Webフックとユーザー定義の Webフックを提供します。
対応する通知受信グループは、**サービス**の通知設定時に使用できます。
...
6. **Webhook設定**
    - 通知送信時に、ユーザーが希望するWebフックに設定できます。
    - タイプ別にデフォルトのWebフックとユーザー定義のWebフックを提供します。（それぞれ最大5個までサポート）

<a id="webhook-settings"></a>
### Webhook設定 { #webhook-settings }
1. コンソールの組織、プロジェクト上部タブで **[通知受信グループ管理]** タブを選択します。
2. 下部の **[Webhook設定]** メニューの **[+Webhook追加]** ボタンを選択します。
3. **[Webhook追加]** ポップアップでは、タイプ別に基本Webhookとカスタムwebhookを提供します。
    - 基本Webhook
        - 基本Webhookは、対応しているサービスの通知時にのみ送信できます。
        - Webhook名
            - 設定する基本Webhookの名前を、文字制限なしで最大40文字以内で入力できます。
            - 同じ通知受信グループ内のWebhook名は重複できません。
        - 受信先
            - Webhookを受信するURLを入力できます。
        - シークレットキー
            - シークレットキーは、URIとid、source、type、timeをSHA256ハッシュアルゴリズムで暗号化するためのシークレットキーです。
                - シークレットキーが設定されている場合にのみ、基本WebhookのHeaderにAuthorizationを含めて送信します。
                - 例: Authorization: HMAC-SHA256 Signature={暗号化した文字}"
                    - Authorization Headerを確認することで、受信したリクエストが中間攻撃者によって改ざんされることを防ぐことができます。
        - HTTP MethodはPOSTで、Request Bodyは以下の形式で提供されます。bodyフィールドはサービスごとに異なります。
...
- カスタムWebフック
    - カスタムWebフックは、対応するサービスの通知時にのみ送信できます。
    - Webフック名
        - 設定するカスタムWebフックの名前を最大40文字で入力できます。
        * 同じ通知受信グループ内でWebフック名は重複できません。
    * サービス
        * Webフックを送信するサービスを選択できます。
        * カスタムWebフックを提供するサービスのみ選択できます。
    * イベント
        * 選択したサービスが提供するイベントのうち、Webフックで受信するイベントを選択できます。
    * 受信対象
        * WebフックのURLとHTTP Methodを入力できます。
...
- リクエストデータ
      - リクエストデータには、選択したサービスとイベントが提供するパラメータを入力できます。
          - Ctrl キー + スペースバーを押して、該当するサービスとイベントがサポートするパラメータを確認できます。
          - 右下のプレビューから提供される Value を確認できます。
              - \"alertId\": \"${alertId}\", <br> \"orgName\" : \"${orgName}\"

- リクエストヘッダ
    - 受信対象に転送するヘッダを入力できます。
    - 受信対象に転送する Content-type ヘッダは application/json をサポートしており、変更することはできません。
...
名前、サービス、イベント、受信対象を入力した後、**[確認]** ボタンを押してWebフックを作成します。
