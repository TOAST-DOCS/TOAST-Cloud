...
* 個人情報保護設定
    * 組織/プロジェクト > メンバー管理 > NHN Cloud アカウント / IAM アカウント > メンバーリストダウンロード機能
        ...
        * 設定を選択した場合、メンバーリストダウンロード機能が無効化され、例外的に許可されたIPまたはIP帯域からのみメンバーリストのダウンロードが可能です。
    * 組織 > CloudTrail > 個人情報
        * 設定なしを選択した場合、ログリストを照会できるすべてのメンバーに、ログリスト内のすべての情報を提供します。
        * 設定を選択した場合、ログリスト内の個人情報（メールアドレス、名前、ID）がマスキング処理されて提供されます。
