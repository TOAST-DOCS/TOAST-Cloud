<!-- pre-align:aligned sig=15db7b09ad9d -->
...
| node_count               | Integer | O   | 	全体ワーカーノード数                          |
| addons                   | Object  | -   | インストールするアドオン情報一覧。複数インストールする場合は繰り返し入力 |
| addons.name              | String  | O   | アドオン名                               |
| addons.version           | String  | O   | アドオンバージョン                                |
| addons.options           | String  | -   | アドオンごとのオプション                              |
...
| labels.boot_volume_type  | String  | O   | 基本ワーカーノードグループ適用：ブロックストレージの種類           |
| labels.boot_volume_size  | String  | O   | 基本ワーカーノードグループ適用:ブロックストレージサイズ(GB)      |
| labels.ca_enable  | String  | O   | 基本ワーカーノードグループ適用：クラスターオートスケーラー機能の有効化有無<br>("True" / "False")   |
| labels.cert_manager_api  | String  | O   | CSR(Certificate Signing Request)機能の有効化有無。必ず「True」に設定     |
