## NHN Cloud > NHN Cloud SDK 使用ガイド > NHN Cloud Log & Crash > Unity
...
NHN CloudLogger を初期化すると、モバイル環境でクラッシュが発生した場合、または Unity で予期せぬ例外が発生した場合、自動的にクラッシュログが送信されます。
クラッシュログの送信を無効化したい場合は、下記のように NHN CloudLoggerConfiguration オブジェクトの EnableCrashReporter プロパティを false に設定してください。
