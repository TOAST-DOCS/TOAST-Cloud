...
* 最小サポートバージョン引き上げ
    * Android最小サポートバージョンがAPI 16(Android 4.1)からAPI 21(Android 5.0)に引き上げられました。
    * NHN Cloud Android SDKで使用されている一部のAPIは、TLS 1.0およびTLS 1.1のサポートが近日中に終了し、TLS 1.2以降のみがサポートされます。
    * これはセキュリティ強化のための措置であり、SDKをアップデートしない場合でも、v1.12.0未満のバージョンをAndroid 5.0（API 21）未満の環境で使用すると、一部のAPIが正常に動作しない可能性があります。
* 16KBページサイズのサポート
