...
| projectName | String | プロジェクト名 |
| country | String | 国コード |
| usagePrice | Long | 利用料金 |
| contractUsagePrice | Long | 約定割引/割増が適用された利用料金合計 |
...
| prePaidTotalAmount | Long | 前払い利用料金 |
| totalCredit | Long | クレジット最終金額 |
| creditUsages | List&lt;CreditUsageProtocol&gt; | クレジット使用金額 |
...
| projectName | String | プロジェクト名 |
| adjustment | Long | 割引/割増金額 |
| adjustmentTypeCode | String | 割引/割増タイプ<br>- CONTRACT_EXTRA: 約定割増<br>- CONTRACT_PENALTY: 約定違約金<br>- CONTRACT_DISCOUNT: 約定割引<br>- CONTRACT_PAYBACK: パートナーペイバック<br>- STATIC_EXTRA: 静的金額割増<br>- PERCENT_DISCOUNT: パーセント割引<br>- COUPON: クーポン<br>- STATIC_DISCOUNT: 静的金額割引<br>- CUTOFF: 500ウォン未満切捨て |
...
| usagePrice | Long | 利用料金合計 |
| totalPrice | Long | 約定割引が適用された利用料金合計 |
| totalDiscount | Long | 割引金額合計 |
| prePaidTotalAmount | Long | 前払い利用料金 |
...
| --- | --- | --- |
| uuid | String | 会員UUID |
| autoPaymentTypeCode | String | 決済手段タイプ<br><br>- PAYCO_CREDIT_CARD: PAYCOクレジットカード<br>- CREDIT_CARD: クレジットカード<br>- INTER_CREDIT_CARD: 海外クレジットカード<br>- UNION_PAY: ユニオンペイ<br>- JAPAN_BILLING: 日本ビリング<br>- ACCOUNT_TRANSFER: 口座振替<br>- CREDIT_ALL: 一般クレジット<br>- CREDIT_LIMIT: イベントクレジット<br>- ESM: 内部費用<br>- ONETIME_PAYMENT: 一回決済<br>- TAX_BILL: 税金計算書発行<br>- CONTRACT_BILL: 税金計算書発行（別途契約による請求金額調整あり）<br>- NONE: なし |
...
| receiptStatusCode | String | 売上伝票ステータスコード<br><br>- NONE: まだ経理部門へ売上報告が行われておらず、売上伝票を確認できない状態<br>- EXIST: 最終金額調整完了後、経理部門へ売上報告が行われ、売上伝票を確認できる状態 |
| refundAccountRegisterStatusCode | String | 返金口座登録状況<br><br>- ALLOW: 返金口座登録オープン状態<br>- DENY: デフォルト、返金口座登録クローズ状態 |
...
| totalDiscount | Long | 割引金額合計 |
| prePaidTotalAmount | Long | 前払い利用金額 |
| totalItems | Integer | UsageGroup別Usageの総件数 |
| usages | List&lt;Usage&gt; | 詳細使用量一覧 |
| usageResourceGroups | List&lt;UsageResourceGroup&gt; | グループ化された使用量一覧 |
...
| --- | --- | --- |
| adjustment | Long | 割引/割増金額 |
| adjustmentTypeCode | String | 割引/割増タイプ<br><br>- CONTRACT_EXTRA: 約定割増<br>- CONTRACT_PENALTY: 約定違約金<br>- CONTRACT_DISCOUNT: 約定割引<br>- CONTRACT_PAYBACK: パートナーペイバック<br>- STATIC_EXTRA: 固定金額割増<br>- PERCENT_DISCOUNT: パーセント割引<br>- COUPON: クーポン<br>- STATIC_DISCOUNT: 固定金額割引<br>- CUTOFF: 500ウォン未満切り捨て |
| description | String | 割引/割増内容 |
...
| 16500 | 非同期タスクが見つかりません | 不正な非同期IDでリクエストしていないかを確認してください。 |
...
| 22007 | 当該パートナーにアクセス権限がありません | パートナーが当該リソースにアクセスする権限がない状態です。照会対象のリソースが自身のリソースであるか、または自身が照会可能な対象のリソースであるかを確認してください。 |
| 22008 | 当該サービスのアプリキーではありません | 誤ったアプリキーでリクエストしていないか確認してください。 |
| 22009 | 当該サービスのカウンター名ではありません | 誤ったカウンター名でリクエストしていないかを確認してください。 |
